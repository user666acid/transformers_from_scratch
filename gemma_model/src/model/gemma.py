...
class Gemma(nn.Module):
    def __init__(self, 
        ...
        self.decoder = nn.Sequential(
            *[Decoder(num_q_heads, emb_size, head_size, max_seq_len, self.rope) for _ in range(num_layers)]
        )
    ...
    def forward(self,
        ...
        for i, decoder_layer in enumerate(self.decoder):
            ...
            emb, layer_cache = decoder_layer(emb, use_cache, layer_cache)
            decoder_caches.append(layer_cache)
            
        emb = self.norm(emb)
        logits = self.proj(emb)
        
        return (logits, decoder_caches)
