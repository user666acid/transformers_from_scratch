...
class HeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        if use_cache:
            if cache:
                K_rotated = torch.cat([cache[0], K_rotated], dim=1)
...
class MultiHeadAttention(nn.Module):
    def __init__(self,
        ...
        self.attn_heads = nn.ModuleList([HeadAttention(emb_size, head_size, max_seq_len, rope) for _ in range(num_heads)])
